<template>
  <div class="ym-tree-wrapper">
    ...
    <a-tree
      ...
      v-model:selected-keys="selfSelectedKeys"
...
// 展开/收起key集合
const selfExpandedKeys = ref<string[] | number[]>([])

// 选择节点集合
const selfSelectedKeys = ref<string[] | number[]>([])
...
watchEffect(() => {
  if (props.selectedKeys.length > 0) {
    selfSelectedKeys.value = props.selectedKeys
  } else {
    const firstNode = props.treeData[0]
    // 设置第一次
    selfSelectedKeys.value = firstNode ? [firstNode[keyField.value]] : [-1]
  }
  if (props.expandedKeys.length > 0) {
    selfExpandedKeys.value = props.expandedKeys
  }
})
...
watch(searchValue, (value) => {
  ...
  selfExpandedKeys.value = expanded
...
const onExpand = (keys: number[]) => {
  selfExpandedKeys.value = keys
  autoExpandParent.value = false
}

const onSelected = (keys: number[], event: any) => {
  selfSelectedKeys.value = keys
