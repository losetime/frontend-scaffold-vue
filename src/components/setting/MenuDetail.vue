<template>
  ...
  >
    <a-form :label-col="labelCol" labelAlign="left">
      ...
      <a-form-item :label="formatMenuName()" v-bind="validateInfos.menuName">
        <a-input v-model:value="detailInfo.menuName" :placeholder="`请输入${formatMenuName()}`" />
      </a-form-item>
 ...
 * @desc 格式化名称
 */
const formatMenuName = () => {
  switch (detailInfo.menuType) {
    case 'M':
      return '目录名称'
    case 'C':
      return '菜单名称'
 ...
 */
const handleConfirm = () => {
  const otherField = ['component', 'path', 'routerName', 'level']
  let validateField = Object.keys(toRaw(detailInfo)).filter((val: string) => !otherField.includes(val))
  switch (detailInfo.menuType) {
    case 'M':
      validateField = validateField.concat(['path', 'routerName', 'level'])
      break
    case 'C':
      validateField = validateField.concat(otherField)
      break
  }
